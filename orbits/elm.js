...
var author$project$Main$RungeKutta = {$: 'RungeKutta'};
	...
	return _Utils_Tuple2(
		{
			clear: false,
			count: 0.0,
			integrationMethod: author$project$Main$RungeKutta,
			objects: _List_fromArray(
				[
					{
					coordinates: {x: 30.0, y: 0.0},
					integratedData: _List_Nil,
					mass: 100.0,
					name: 'obj1',
					velocity: {x: 0.0, y: 18.0}
				},
					{
					coordinates: {x: 30.7, y: 0.0},
					integratedData: _List_Nil,
					mass: 1.0,
					name: 'obj2',
					velocity: {x: 0, y: 14.2}
	...
	return elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				A2(elm$time$Time$every, 0, author$project$Main$Tick)
			]));
};
var author$project$Main$Integrate = function (a) {
...
var author$project$Main$vectorAbs = function (vector) {
	return elm$core$Basics$sqrt(
		A2(elm$core$Basics$pow, vector.x, 2) + A2(elm$core$Basics$pow, vector.y, 2));
};
...
var author$project$Main$calculateAcceleration = F2(
	function (object1, listOfOtherObjects) {
		if (!listOfOtherObjects.b) {
			...
			var r2 = h.coordinates;
			var r1 = object1.coordinates;
...
var author$project$Main$rungeKuttaIntegrateObj = F3(
	function (listOfOtherObjects, tObject, dt) {
		var v0 = tObject.velocity;
		var t0 = 0;
		var t1 = t0 + (dt / 2);
		var t2 = t0 + (dt / 2);
		var t3 = t0 + dt;
		var r0 = tObject.coordinates;
		var f2 = F3(
			function (t, r, v) {
				return A2(
		...
		return _Utils_update(
			tObject,
			{coordinates: newCoordinates, velocity: newVelocity});
	});
			...
			switch (msg.$) {
				...
				case 'Tick':
					...
					var updatedModel = _Utils_update(
						model,
						{
							objects: A2(author$project$Main$integrateAllObjects, 1 / 2000, model.objects)
						});
					if (A2(
						elm$core$List$any,
						function (o) {
							return elm$core$List$isEmpty(o.integratedData);
						},
						model.objects)) {
						var $temp$msg = author$project$Main$Integrate(100),
...
var author$project$Main$ToggleClear = {$: 'ToggleClear'};
	...
	if (!objectList.b) {
		...
		return A2(
			...
			A2(
				...
				_Utils_Tuple2(h.coordinates.x * 10, h.coordinates.y * 10),
				elm$core$Basics$sqrt(
					elm$core$Basics$sqrt(h.mass))),
...
var author$project$Main$render = function (model) {
	...
	var x = -(size / 2);
	var y = -(size / 2);
	var rotation = elm$core$Basics$degrees(model.count * 3);
	var hue = A2(
		elm$core$Basics$modBy,
		100,
		elm$core$Basics$floor(model.count / 4)) / 100;
	return A2(
		joakin$elm_canvas$Canvas$shapes,
		_List_fromArray(
			[
		...
		author$project$Main$drawObjects(model.objects));
};
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text(
								model.clear ? 'Leave traces' : 'Refresh screen')
							])),
						elm$html$Html$text(
						'Current integration method: ' + author$project$Main$integrationMethodToString(model.integrationMethod))
					])),
				A3(
